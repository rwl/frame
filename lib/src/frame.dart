...
abstract class Frame<Row, Col> {
  ...
  /// This is largely for optimization purposes.
  bool isColOriented;
  ...
  /// Transposes the rows and columns of this frame. All rows in this
  /// frame becomes the columns of the new frame.
...
//      case (id, Value(column)) if Series(rowIndex, column.cast[Any]).hasValues => id
  ...
  /// The following methods allow a user to apply reducers directly across a
  /// frame. In particular, this API demands that we specify the type that the
  /// reducer accepts and it will only apply it in the case that there exists
  /// a type conversion for a given column.
...
//    }
  ...
  /// Put the columns in sorted order. This affects only the traversal order
  /// of the columns.
  Frame<Row, Col> sortColumns() => withColIndex(colIndex.sorted);

  /// Put the rows in sorted order. This affects only the traversal order
  /// of the rows.
  ...
  /// Map the row index using [f]. This retains the traversal order of
  /// the rows.
...
//    } yield a
  ...
  /// column key using [f] and stores it in the row `to`. If [to] doesn't
  /// exist then a new row will be appended onto the frame, otherwise the
  /// row(s) with key [to] will be removed first.
  Frame<Row, Col> mapWithIndex(
          Rows<Row, A> rows, Row to, f(Tuple2<Col, A> a)) =>
      transpose.mapWithIndex(rows.toCols, to)(f).transpose();

  /// Maps each column to a value using [cols], then maps the result with the
  /// row key using [f`] and stores it in the column [to]. If [to] doesn't
  /// exist then a new column will be appended onto the frame, otherwise the
  ...
  /// this frame as the row [to]. If [to] doesn't exist then a new row will
  /// be appended onto the frame, otherwise the row(s) with key [to] will be
  /// removed first.
  ...
  /// for a given column, [p] is false, then that column will be removed from
  /// the frame. Any [NA] and [NM] columns will also be removed.
  ...
  /// `frame.sortBy(cols).reindex(cols)`.
  ...
  ///
  /// This is equivalent to, but more efficient than,
  /// `frame.sortBy(rows).reindex(rows)`.
  ...
  ///
  /// ```
  /// > val f = Frame.fromColumns("a" :: 2 :: HNil, "b" :: 3 :: HNil)
  /// > f.reduce(Cols(1).as[Int], 1)(reduce.Sum)
  /// res0: Frame<int, int> =
  ///     0 . 1
  /// 0 : a | b
  /// 1 : 5 | 5
  ...
  /// duplicated for each row. If [to] exists it will be replaced, otherwise
  /// it will be added to the end of the columns.
...
//  }
  ...
  ///
  ///     def mergeColumn<Row, Col>(cols: (Col, Series[Row, _])*): Frame<Row, Col>
...
//    gen.to(cols).foldLeft(new Frame.empty<Row, Col>, mergeSeries)
  ...
  /// than having to use explicit `HList`s.
  ///
  ///     def mergeRows<Row, Col>(rows: (Row, Series[Col, _])*): Frame<Row, Col>
