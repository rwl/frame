...
class CellGenerator<A extends Comparable> {
  Function _rand;
  CellGenerator(A rand()) : _rand = rand;
  ...
  /** Generate a cell that can produce one of [[Value]], [[NA]] or [[NM]] cells based on a given
    ...
    */
  Cell<A> genCell([int weightValue = 60, int weightNA = 3, int weightNM = 1]) {
    var rval = r.nextInt(weightValue + weightNA + weightNM);
    if (rval < weightValue) {
      return new Value(_rand());
    } else if (rval >= weightValue && rval < weightNA) {
      return NA;
    } else {
      return NM;
    }
...
class ColumnGenerators<A> {
  /** A generator for a higher-order arbitrary column that generates a column who has arbitrary
    * values
    */
//  implicit def arbColumn[A: Arbitrary: ClassTag]: Arbitrary[Column<A>] = Arbitrary(genColumn(arbitrary<A>))
  ...
  /** Generate a [[framian.Column]] with the provided distribution of cell types.
    ...
    */
  Column<A> genColumn(
      [int weightValue = 60, int weightNA = 3, int weightNM = 1]) {
    var cellValues = new List<Cell<A>>.generate(
        maxl, (i) => cg.genCell(weightValue, weightNA, weightNM));
    return new Column(cellValues);
  }
}
