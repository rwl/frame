...
maskTest() {
  Mask genMask() {
    var rows0 = new List<int>.generate(r.nextInt(maxl), (_) => 0);
...
//  implicit val arbMask: Arbitrary[Mask] = Arbitrary(genMask)
  ...
  Filter genFilter() {
    List<int> mods0 = new List<int>.generate(r.nextInt(maxl), (_) => 0);
...
/*
  ...
  group("filter should", () {
    test("remove bits that are filtered out", () {
      expect(mask.filter(filter).toSet(), equals(mask.toSet().filter(filter)));
    });
  });
  ...
  group("min should", () {
    test("return minimum set bit", () {
      expect(
          mask.toSet().reduceOption((a, b) => min(a, b)), equals(mask.min()));
    });

    test("return None for empty Mask", () {
      expect(new Mask.empty().min(), equals(None));
    });
  });
  ...
  group("max should", () {
    test("return maximum set bit", () {
      expect(
          mask.toSet().reduceOption((a, b) => max(a, b)), equals(mask.max()));
    });
  ...
  group("| should", () {
    ...
    test("contain all bits from both arguments", () {
      ...
      expect(bits.forall(mask), isTrue);
    });
  });
  ...
  group("& should", () {
    test("only contain bits contained in both arguments", () {
      var mask = (a & b);
      expect(mask.toSet().forall((i) => a(i) && b(i)), isTrue);
    });

    test("contain all bits that are in both arguments", () {
      var mask = a & b;
      var bits = a.toSet() & b.toSet();
      expect(bits.forall(mask), isTrue);
    });

    test("shrink array if top zero'd out", () {
      var a = new Mask(1, 100);
      var b = new Mask(1, 101);
      expect((a & b).max(), equals(Some(1)));
    });
  });
  ...
  group("-- should", () {
    test("not contain bits in right-hand side", () {
      var mask = a.dec(b);
      expect(b.toSet().forall((i) => !mask(i)), isTrue);
    });

    test("contain bits in the lhs but not the rhs", () {
      var mask = a.dec(b);
      var setBits = a.toSet().dec(b.toSet());
      expect(setBits.forall(mask), isTrue);
    });
  });

  group("equals should", () {
    test("always be equal for equivalent masks", () {
      var b = new Mask(a.toSet().toSeq());
      expect(a, equals(b));
    });

    test("should not throw IOOE when size are equal but lengths are not", () {
      var a = new Mask(1, 2, 3);
      var b = new Mask(1000, 1001, 2000);
      expect((a == b), not(throws));
      expect((b == a), not(throws));
    });
  });
   */
}
