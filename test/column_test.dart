...
columnTest() {
  Vector<Cell<A>> slice(Column<A> col, Iterable<int> indices) =>
      indices.toVector().map((i) => col(i));

  Column<A> mkEval(Column<A> col) => Column.eval((row) => col(row));
...
//  implicit def arbColumn[A: Arbitrary]: Arbitrary[Column<A>] = Arbitrary(genColumn(arbitrary<A>))
  ...
  group("Column construction", () {
    test("wrap arrays", () {
      var col = Column.dense(Array(1, 2, 3));
      expect(slice(col, [-1, 0, 1, 2, 3]),
          equals([NA, Value(1), Value(2), Value(3), NA]));
    });

    test("constructable from Cells", () {
      var col = new Column.fromCells([NM, NA, Value("a"), NM]);
      expect(slice(col, [-1, 0, 1, 2, 3, 4]),
          equals([NA, NM, NA, Value("a"), NM, NA]));
    });

    test("wrap row function", () {
      var col = Column.eval((row) => Value(-row));
      expect(col.apply(0), equals(Value(0)));
      expect(col.apply(Int.MinValue), equals(Value(Int.MinValue)));
      expect(col.apply(Int.MaxValue), equals(Value(-Int.MaxValue)));
      expect(col.apply(5), equals(Value(-5)));
    });

    test("empty is empty", () {
      var col = Column.empty /*[String]*/ ();
      expect(slice(col, [Int.MinValue, 0, Int.MaxValue, -1, 1, 200]),
          contains(NA));
    });
  });
...
//      (col: Column[Int], indices: List[Int]) =>
  ...
  group("reindex", () {
    ...
    test("reindex with negative indices", () {
      ...
      expect(
          col.reindex([-1, 0, -1]).slice(range(3)), equals([NA, Value(1), NA]));
    });
  });
...
//      (a: Column[Int], b: Column[Double], indices: List[Int]) =>
      ...
      expect(indices.map((i) => col(i)),
          equals(indices.map((row) => a(row).zipMap(b(row), (a, b) => a + b))));
    });
  });
...
denseColumnTest() {
  ...
  group("dense columns", () {
    test("manually spec from dense constructor", () {
      expect(Column.dense(["1", "2", "3"]), new isInstanceOf<GenericColumn>());
    ...
    test("force manual spec through flatMap", () {
      ...
      expect(
          col.flatMap((n) => Value(n.toInt())), new isInstanceOf<IntColumn>());
    ...
    test("retain manual spec through force", () {
      ...
      expect(Column.dense([1.0, 2.0, 3.0]).force(2),
          new isInstanceOf<DoubleColumn>());
      expect(Column.dense([1, 2, 3]).force(2), new isInstanceOf<LongColumn>());
    });

    test("retain manual spec through mask", () {
      var mask = Mask(1);
      expect(Column.dense([1, 2, 3]).mask(mask), new isInstanceOf<IntColumn>());
